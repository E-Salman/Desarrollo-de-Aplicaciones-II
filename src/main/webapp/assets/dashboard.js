(() => {
  ...
  function renderResumen(r) { ultimoResumen = r; $("capital").textContent = money(r.capitalInvertido); $("patrimonio").textContent = money(r.patrimonioTotal); $("ganancia").textContent = money(r.gananciaTotal); $("posiciones").innerHTML = r.posiciones.length ? r.posiciones.map(p => `<article class="posicion"><div><b>${p.nombre}</b><small>${p.ticker} · ${p.tipo}</small><small>Cantidad: ${p.cantidad} · Promedio: ${money(p.precioPromedio)}</small></div><div><b>${money(p.actual)}</b><small class="${dec(p.rendimiento) >= 0 ? "positivo" : "negativo"}">${dec(p.rendimiento) >= 0 ? "+" : ""}${money(p.rendimiento)} (${dec(p.rendimientoPorcentaje).toFixed(2)}%)</small></div></article>`).join("") : "<p>Aún no hay inversiones.</p>"; }
  async function cargarResumen() { const r = await fetch(ctx + "/api/portfolio/resumen"); renderResumen(await r.json()); }
